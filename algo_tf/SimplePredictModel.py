...
from BackModels import TimeAwareRNN, PredictMLP, PNN
...
class SimplePredictModel(tf.keras.layers.Layer):
    def __init__(self, all_features_num, input_dim, hidden_dim, predict_hidden_width,
        ...
        super(SimplePredictModel, self).__init__()
        ...
        self.pnn = PNN(w1, self.embedding_matrix)
        self.pnn_target = PNN(w1, self.embedding_matrix)
    ...
    def forward(self, inputs):
        ...
        # target_item: (bs,fields)
        x, x_continuous, target_item, self_loc, y = inputs
        if not self.hard_search:
            x = self.get_top_k(x, self_loc)  # (bs,item_num,fields+2)
        ...
        target_item = self.pnn_target(target_item)

        h = tf.concat([target_item, h, h_continuous], axis=-1)  # (bs,input_size+2*hidden_size)
